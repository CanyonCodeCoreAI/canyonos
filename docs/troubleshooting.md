...
**When:** an approved `canyonos deploy` fails during build, startup, or a
request.

**How:** find the symptom, verify the likely cause, then follow the linked
canonical reference. For runtime mechanisms read
[runtime-contract](runtime-contract.md); for proxy or remote-host failures
read [llm-proxy](llm-proxy.md) or [ec2](ec2.md).
...
| Source module is missing | `.car/app` is not rooted at the source's import root, so the original import does not resolve from `/app`; see [images-and-dependencies](images-and-dependencies.md) |
...
| Symptom | Likely cause |
|---|---|
| Ctrl+C leaves the deployment running | `canyonos deploy` follows logs; Ctrl+C stops monitoring, not the deployment |
