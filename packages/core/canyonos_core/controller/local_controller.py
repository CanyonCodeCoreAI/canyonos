...
class LocalController(object):
    ...
    def __init__(self, port=50051, publish_ready=True):
        ...
        # mark_stopped may move it. It starts as "starting" because a launcher
        # that owns readiness (publish_ready=False) decides after the heartbeat
        # is already beating, and a container is ready only once something has
        # said so.
        ...
        logger.info(
            "Local controller initialized at %s (max_agent_instances=%d), status %s.",
            self._my_endpoint,
            max_instances,
            self._status,
        )
    ...
    def _agent_declared(self):
        ...
        that failed to import. Only the second is a failure, and half a
        declaration counts as one: it can never load.
        """
        return bool(self.agent_name or self.agent_file)

    def _die_unloadable(self):
        """Mark failed and raise: a configured agent that did not load is fatal.
        ...
        unreachable status key cannot mask the cause or leave a half-dead
        container listening.
        """
        try:
            self.mark_failed()
