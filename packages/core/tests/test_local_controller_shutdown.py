...
class LocalControllerShutdownTests(unittest.TestCase):
    def test_stop_returns_when_executor_work_does_not_finish(self):
        ...
        # stop() publishes through the real status methods.
        controller.mark_stopped = MethodType(LocalController.mark_stopped, controller)
        controller._publish_status = MethodType(
            LocalController._publish_status, controller
        )
