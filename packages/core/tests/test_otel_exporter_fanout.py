...
# therefore imports ``trace_convert`` and ``otel_reader`` as top-level modules.
...
class OTelExporterFanoutTests(unittest.TestCase):
    ...
    def _assert_row_unsent(self):
        ...
        try:
            self.assertEqual(
                conn.execute("SELECT sent FROM traces_waiting").fetchone()[0], 0
            )
        finally:
            conn.close()
    ...
    def test_send_pending_delivers_the_same_spans_to_every_destination(self):
        ...
        with (
            patch.object(otel_exporter, "DB_PATH", self.db_path),
            patch.object(
    ...
    def test_send_pending_leaves_rows_unsent_when_a_destination_returns_failure(self):
        ...
        with patch.object(otel_exporter, "DB_PATH", self.db_path):
...
class OTelExporterQuietFailureTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        otel_writer._cost_failures_logged.clear()
    ...
    def test_cost_lookup_failure_is_logged_once_per_kind_and_keeps_the_row(self):
        ...
        with (
            patch.object(otel_writer.pricing, "compute_token_cost", side_effect=boom),
            patch.object(otel_writer.pricing, "compute_server_cost", side_effect=boom),
            self.assertLogs(otel_writer.logger, level="WARNING") as captured,
        ):
...
class OTelExporterRowFidelityTests(unittest.TestCase):
    ...
    def test_empty_output_is_preserved_rather_than_treated_as_absent(self):
        self.assertEqual(otel_writer._normalize_json_text(""), '""')
        self.assertIsNone(otel_writer._normalize_json_text(None))
...
class OTelExporterPlaceholderTests(unittest.TestCase):
    ...
    def test_placeholder_is_sent_only_after_the_attempt_limit(self):
        ...
        try:
            self.assertEqual(
                conn.execute("SELECT sent FROM traces_waiting").fetchone()[0], 1
            )
        finally:
            conn.close()
...
class OTelExporterLifecycleLoggingTests(unittest.TestCase):
    ...
    def test_recovery_is_logged_only_after_a_failure(self):
        ...
        otel_exporter._trace_exporters = [("live", exporter)]
        with patch.object(otel_exporter, "DB_PATH", db_path):
            otel_exporter._trace_send_pending()
    ...
    def test_steady_success_does_not_log_recovery(self):
        ...
        with (
            patch.object(otel_exporter, "DB_PATH", db_path),
