...
def test_a_deploy_that_never_came_up_exits_non_zero(monkeypatch, deployable):
    ...
    `deploy` subcommand has to carry the tail's failure all the way out.
    """
    import cli as cli_entry

    def never_came_up(*_a, **_k):
        raise RuntimeError("Deploy stopped before the workflow became ready.")

    monkeypatch.setattr(deploy_cmd, "_stream_logs_and_autoserve", never_came_up)
